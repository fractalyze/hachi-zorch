...
Tables are dense hypercube evaluations, variable 0 on the least-significant
index bit; binding variable 0 at `t` sends `T[y] -> T[2y] + t*(T[2y+1]-T[2y])`,
so the round polynomial of a two-factor product has closed-form ascending
coefficients
...
def _round_coeffs(a: Array, bb: Array) -> Array:
    ...
    b0, db = _split(bb)
    return jnp.stack(
        [jnp.sum(a0 * b0), jnp.sum(a0 * db + b0 * da), jnp.sum(da * db)]
    )
...
def prove_paired_sumcheck(
    ...
    (each `2^m0` entries), `rs` the shared challenges. Returns
    `(claims, msgs)`: the initial claim pair `[sum f0, sum fa]` and the
    `(m0, 6)` coefficient rows (`g0` then `ga`, ascending)."""
    a, b = f0
    c, d = fa
    claims = jnp.stack([jnp.sum(a * b), jnp.sum(c * d)])
    rows = []
    for i in range(rs.shape[0]):
        rows.append(jnp.concatenate([_round_coeffs(a, b), _round_coeffs(c, d)]))
        a, b = _bind(a, rs[i]), _bind(b, rs[i])
        c, d = _bind(c, rs[i]), _bind(d, rs[i])
    return claims, jnp.stack(rows)
...
def final_evals(
    ...
    return jnp.stack([a[0] * b[0], c[0] * d[0]])
